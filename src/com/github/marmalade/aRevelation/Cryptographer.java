...
public class Cryptographer {
    ...
    public static String decrypt(byte[] fileData, String password) throws WrongPasswordException,
            MissingDecryptorException, UnknownFileFormatException, InvalidDataException {
        byte[] header;
        ...
        if (Arrays.equals(Arrays.copyOfRange(header, 0, 6), MAGIC_STRING_DATA_VERSION_2)) {
            if (Arrays.equals(VERSION_0_4_7, Arrays.copyOfRange(header, 6, 9))) {
                ...
                byte[] hash256;
                try {
                    cypher = Cipher.getInstance("AES/CBC/PKCS5Padding");
                    ...
                    try {
                        compressedData = cypher.doFinal(input);

                    } catch (BadPaddingException e) {
                        throw new WrongPasswordException();
                    }

                    hash256 = Arrays.copyOfRange(compressedData, 0, 32);
    ...
    private static byte[] decompress(byte[] inputData) throws InvalidDataException {
